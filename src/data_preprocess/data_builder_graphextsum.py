...
from data_utils import _get_word_ngrams
from data_preprocess.roberta.roberta_tokenization import GptBpeTokenizer
import nltk
import itertools
import time
...
def format_to_json_infernce(args):
    """Constuct json format dataset"""

    # for corpus_type in ["test"]:
    corpus_type = "test"
    src_files = codecs.open(args.test_src, 'r', 'utf-8').readlines()
    tgt_files = codecs.open(args.test_tgt, 'r', 'utf-8').readlines()
...
def _format_to_json_infernce(params):
    """Constuct json format dataset"""
    src, args = params
...
def format_to_paddle_infernce(args):
