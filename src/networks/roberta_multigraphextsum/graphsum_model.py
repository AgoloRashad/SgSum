...
class GraphSumModel(object):
    ...
    def ext_predict(self, pyreader_name, is_prediction=False):
        ...
        score_sorted = layers.argsort(input=reshaped_correct_scores, axis=1, descending=True)[0]
        layers.Print(score_sorted, message="score_sorted", summarize=-1)
        cand_summary_sent_ids = score_rank[:,:self.c_sent_num,]
        layers.Print(cand_summary_sent_ids, message="cand_summary_sent_ids", summarize=-1)
